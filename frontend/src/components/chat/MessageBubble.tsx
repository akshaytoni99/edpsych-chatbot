    ...
    >
      ...
      <div
        className={`max-w-[78%] sm:max-w-[72%] group relative ${
          isUser
            ? 'bg-gradient-to-br from-indigo-500 via-indigo-500 to-indigo-600 text-white rounded-2xl rounded-br-md shadow-lg shadow-indigo-200/30'
            : isValidation
      ...
      >
        ...
        {!isUser && !isValidation && (
          <div className="absolute inset-0 rounded-2xl rounded-bl-md bg-gradient-to-br from-indigo-50/30 via-transparent to-purple-50/20 pointer-events-none" />
        )}
        ...
        <div className="relative px-4 py-3">
          ...
          <div className={`flex items-center gap-1.5 mt-1.5 ${isUser ? 'justify-end' : 'justify-start'}`}>
            <p className={`text-[10px] font-medium ${
              isUser ? 'text-indigo-200/60' : isValidation ? 'text-amber-400/70' : 'text-gray-300'
            }`}>
              {timeString}
            </p>
            {/* Sent indicator for user messages */}
            {isUser && (
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" className="w-3 h-3 text-indigo-200/50">
